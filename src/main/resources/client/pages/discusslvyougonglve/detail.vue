...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusslvyougonglveCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusslvyougonglveCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discusslvyougonglve', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusslvyougonglve');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discusslvyougonglve');
			...
			async init(){
				...
				let res = await this.$api.info('discusslvyougonglve', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusslvyougonglve', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscusslvyougonglve/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
