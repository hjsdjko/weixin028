	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusstesemeishi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusstesemeishi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusstesemeishi`, this.ruleForm);
						}else{
							await this.$api.add(`discusstesemeishi`, this.ruleForm);
						}
						uni.setStorageSync('discusstesemeishiCleanType',true);
